...
The capability system tells the iOS host what each extension can do, <Capability name="discover" />, <Capability name="filters" />, <Capability name="login" />, etc., by writing a small array into `dist/index.json` at build time. The app uses this to pre-filter sources and gate features without having to load the bundle into a WebView and probe every optional method.
