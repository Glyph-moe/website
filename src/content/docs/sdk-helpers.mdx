...
<Callout type="note">
  Cookies are stored in memory and lost when the app restarts. This is by design for privacy. If a
  site requires login, the user will need to re-authenticate each session.
</Callout>
