...
<Callout type="tip">
  If a request shows status `nil`, it means the request failed before reaching the server (DNS
  failure, TLS error, timeout, or domain blocked by the extension's restriction policy).
</Callout>
...
<Callout type="note">
  `console.log` calls are visible in the app's Developer Tools and in the remote log stream, but NOT
  in Xcode's console. The extension runs in the WASM runtime, not in the app's main process.
</Callout>
