...
Rust support is **experimental**. JS extensions are the recommended path. The Rust pipeline works end-to-end, but the developer experience is bare metal compared to JS, there is no `glyph-rs` SDK crate yet, no test runner, and the example below is the only working reference. The intent is for the community to flesh out helper crates.
</Callout>
